...
CREATE TABLE FONAVI_IUD.DHE_T_HEREDEROS_TMP (
...
CREATE TABLE FONAVI_IUD.RRN_C_ACT_RENIEC (
...
CREATE TABLE FONAVI_IUD.DHE_C_HEREDEROS_GPEC_PERSONA (
                                              DOC_C_TIPO VARCHAR2(20),
                                              DOC_C_NUMERO VARCHAR2(20),
                                              PER_D_APELLIDO_PATERNO VARCHAR2(50),
                                              PER_D_APELLIDO_MATERNO VARCHAR2(50),
...
CREATE OR REPLACE PROCEDURE FONAVI_IUD.SP_DHE_INSERT_COMP_HEREDEROS_MASIVO(
    p_herederos_str IN VARCHAR2
) AS
BEGIN
    -- Eliminar todos los registros de la tabla temporal antes de insertar nuevos registros
    DELETE FROM FONAVI_IUD.DHE_T_HEREDEROS_TMP;
    ...
    FOR rec IN (
        ...
        CONNECT BY LEVEL <= LENGTH(p_herederos_str) - LENGTH(REPLACE(p_herederos_str, '|', '')) + 1
            ...
            INSERT INTO FONAVI_IUD.DHE_T_HEREDEROS_TMP (
    ...
    FROM FONAVI_IUD.DHE_T_HEREDEROS_TMP t
    ...
    DELETE FROM FONAVI_IUD.DHE_C_HEREDEROS_GPEC_PERSONA;

    -- Insertar nuevos registros en la tabla de actualización en GPEC Persona basada en los nuevos registros
    INSERT INTO FONAVI_IUD.DHE_C_HEREDEROS_GPEC_PERSONA (
        DOC_C_TIPO, DOC_C_NUMERO, PER_D_APELLIDO_PATERNO, PER_D_APELLIDO_MATERNO, PER_D_NOMBRES,
    ...
    COMMIT;
END SP_DHE_INSERT_COMP_HEREDEROS_MASIVO;

-- Paso 5: Procedimiento para Actualizar Herederos en RENIEC

CREATE OR REPLACE PROCEDURE FONAVI_IUD.SP_RN_ACTUALIZA_HEREDEROS_RENIEC IS
    CURSOR cur_padron IS
        SELECT *
        FROM FONAVI_IUD.RRN_C_ACT_RENIEC
        WHERE IND_PROCESO = 0;
...
BEGIN
    FOR det_padron IN cur_padron LOOP
            ...
            UPDATE FONAVI_IUD.RRN_C_ACT_RENIEC
            SET IND_PROCESO = 1
...
END SP_RN_ACTUALIZA_HEREDEROS_RENIEC;

-- Paso 6: Procedimiento para Actualizar Datos en GPEC Persona

CREATE OR REPLACE PROCEDURE FONAVI_IUD.SP_GPE_ACTUALIZA_HEREDEROS_PERSONA IS
    CURSOR cur_padron IS
        SELECT *
        FROM FONAVI_IUD.DHE_C_HEREDEROS_GPEC_PERSONA
        WHERE IND_PROCESO = 0;
...
BEGIN
    FOR det_padron IN cur_padron LOOP
            ...
            UPDATE FONAVI_IUD.DHE_C_HEREDEROS_GPEC_PERSONA
            SET IND_PROCESO = 1
            WHERE DOC_C_TIPO = det_padron.DOC_C_TIPO
              AND DOC_C_NUMERO = det_padron.DOC_C_NUMERO;
    ...
    DBMS_OUTPUT.PUT_LINE('FIN ' || n_reg);
END SP_GPE_ACTUALIZA_HEREDEROS_PERSONA;
